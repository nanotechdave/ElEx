...
class H5DataStore:
    ...
    def __fsck(self, fname):
        ...
        for key in [
            ...
            if key not in attrs.keys():
                raise H5FormatError(
                    "File %s does not specify a file format version" % bname
                )
        ...
        for grp in ["crossbar", "crosspoints", "synthetics"]:
            if grp not in self._h5.keys():
                raise H5FormatError(
                    "File %s is missing root base group %s" % (bname, grp)
                )
        ...
        for key in ["words", "bits"]:
            if key not in attrs.keys():
                raise H5FormatError(
                    "File %s does not provide crossbar dimensions" % bname
                )
    ...
    def __create_structure(self, shape, name):
        ...
        for dset in ["voltage", "current"]:
            if dset not in self._h5["crossbar"].keys():
                self._h5["crossbar"].create_dataset(dset, shape=shape, dtype=np.float32)
    ...
    def update_status(
        self, word, bit, current, voltage, pulse, read_voltage, optype=OpType.READ
    ):
        """
    ...
    def update_status_bulk(
        self, word, bit, currents, voltages, pulses, read_voltages, optypes
    ):
        """
        ...
        for a in [voltages, pulses]:
            if len(a) != dlen:
                raise ValueError(
                    """Currents, Voltages and Pulse Widths must have the same """
    ...
    def __sanitise_group_basepath(self, grp, anchor):
        ...
        if pbasepath.anchor == "/":
            ...
            if parent != PurePosixPath(anchor):
                raise KeyError(
                    "Attempting to attach experiment to wrong group node: " + basepath
                )
    ...
    def make_wb_group(self, word, bit, name, tstamp=True):
        ...
        if tstamp:
            ts = time.time_ns()
            grpname = "crosspoints/W%02dB%02d/experiments/%s_%d" % (word, bit, name, ts)
        else:
            ts = None
            grpname = "crosspoints/W%02dB%02d/experiments/%s" % (word, bit, name)

        return self.__make_group([[word, bit]], grpname, ts)

    def make_wb_table(
        self, word, bit, name, shape, dtype, grp=None, maxshape=None, tstamp=True
    ):
        """
    ...
    def make_synthetic_table(
        self, crosspoints, name, shape, dtype, grp=None, maxshape=None, tstamp=True
    ):
        """
    ...
    def __make_table(self, name, shape, dtype, maxshape):
        if maxshape is None:
            ...
            dset = self._h5.create_dataset(
                name, shape=shape, dtype=dtype, maxshape=maxshape, chunks=True
            )
